...
# ------------------ UI ------------------
st.title("Real-Time AI Query Generator")
...
model = genai.GenerativeModel(
    ...
    system_instruction=(
        "You are a helpful assistant. For any given query, provide a brief "
        "summary and include relevant educational YouTube links."
    )
)
...
def extract_youtube_links(text):
    ...
    return list(set(re.findall(youtube_regex, text)))


# ------------------ ACTION ------------------
if st.button("Submit") and query:
    with st.spinner("Generating response..."):
        search_content = search(query)
        answer = model.generate_content(search_content)

        st.subheader("AI Generated Content")
        st.markdown(answer.text)

        links = extract_youtube_links(answer.text)
        if links:
            st.subheader("YouTube Links")
            for link in links:
                st.video(link)
        else:
            st.warning("No YouTube links found.")
